...
class Loss_FEM:
    ...
    def evaluate(
        ...
        if isinstance(stress_field, torch.Tensor):
            stress_field = stress_field.detach()
        ...
        if isinstance(displacement_field, torch.Tensor):
            displacement_field = displacement_field.detach()
        ...
        if isinstance(loaded_boundary_displacement_field, torch.Tensor):
            loaded_boundary_displacement_field = loaded_boundary_displacement_field.detach()
