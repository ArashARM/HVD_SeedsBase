...
class NN_Trainer:
    ...
    def curve_length_similarity_loss(self, decoder_out):
        """
        Encourage selected generated 3D edge curves to have equal lengths.

        The loss is scale-invariant and becomes zero when all selected
        edge lengths are equal.
        """
        cfg = self.cfg

        equal_edge_types = getattr(
            cfg,
        ...
        return log_relative_lengths.square().mean()
